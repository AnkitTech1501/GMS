...
if (isset($_POST['reset'])) {
    ...
    $admin_exists = mysqli_query(
        $con,
        "SELECT EMAIL_ID FROM staffs WHERE username = '$user_input' OR EMAIL = '$user_input'"
    );
    ...
    if (mysqli_num_rows($admin_exists) > 0) {
        ...
        $insert = mysqli_query(
            $con,
            "INSERT INTO password_resets (email, token, role) VALUES ('$email', '$token', 'STAFFS')"
        );

        if ($insert) {
            $resetLink = "http://localhost:8080/Gym-System/staff/reset_password.php?token=$token";
            ...
            try {
                ...
                file_put_contents("StaffsEmaillog.txt", $errorLog, FILE_APPEND);
